...
export default class FixedPointVector extends Int32Array {
  static ONE: number;
    ...
    static SHIFT: number;
    static HALF: number;
  constructor(x: number | FixedPointVector = undefined, y: number = undefined, z: number = undefined) {
  ...
  add(v: FixedPointVector | number[]) {
    this[0] += v[0];
    this[1] += v[1];
    this[2] += v[2];
  }

  sub(v: FixedPointVector | number[]) {
    this[0] -= v[0];
    this[1] -= v[1];
    this[2] -= v[2];
  }
  ...
  copy(v: FixedPointVector | number[]) {
    this[0] = v[0];
    this[1] = v[1];
    this[2] = v[2];
    this[3] = v[3];
...
FixedPointVector.ONE = 1 << FixedPointVector.SHIFT;
FixedPointVector.HALF = FixedPointVector.ONE >> 1;